...
### A1. URI 缺前导 `/`（5 条，全在示例侧）

| section | 标题 | 现示例 URI | 建议 |
|---|---|---|---|
...
| section | 标题 | 差异 |
|---|---|---|
| 4.2.2 / 4.2.3 | 查询/修改指定管理资源信息 | 表格 `{manager_id}` vs 示例 `{SMN(1\|2)}`——示例占位符写法不规范，应统一 `{manager_id}` |
| 4.2.52 / 4.2.53 / 4.2.54 | SP 服务硬盘擦除配置 | 表格路径 `SPService/SPDriveErase` vs 示例路径 `SPService/SPDiagnose/SPDriveErase`——**两条路径不一致，需确认实际 API 路径** |
| 4.2.71 | 删除截屏 | 表格 `DiagnosticService.DeleteScreenShot` vs 示例 `DiagnosticService.CaptureScreenShot`——**Action 名错位** |
| 4.2.102 / 4.2.105 | 启动纳管 / 上载文件 | URI 中 `SMN1(SMN2) /` 后多空格 |
| 4.3.28 | 查询VLAN集合资源信息 | 表格 `Systems/{system_id}/EthernetInterfaces/...` vs 示例 `Systems/{system_id}/Memory/{memory_id}/EthernetInterfaces/...`——**示例多了一段错误路径** |
| 4.3.30 | 配置VLAN | 表格是 POST，示例是 GET；URI 缺 `/` 隔开 `{vlan_id}` 和 `Actions` |
| 4.3.43 | 修改BIOS密码 | 表格 `Bios.ChangePassword` vs 示例 `Bios.ResetBios`——**Action 错位** |
| 4.4.42 | 查询历史功率资源信息 | URI 拼写 `Power.PowerHistoryDat` 缺 `a`（A5） |
| 4.5.6 | 文件上传 | URI 拼写 `FirmwareInventor` 缺 `y`（A5） |
| 4.6.1 | 查询任务服务资源信息 | 表格 `/redfish/v1/TaskService` vs 示例 `/redfish/v1/TaskService/Tasks/{taskid}`——示例写错成单任务路径 |
| 4.9.11 | 查询指定指标报告资源信息 | 示例缺一段（A2 `//` 已含）|

### A8. URI method 不一致（6 条）

| section | 标题 | 表格 method | 示例 method | 建议 |
|---|---|---|---|---|
| 4.2.39 | 创建SP服务的OS安装配置 | GET | POST | 创建用 POST，**改表格** |
| 4.2.69 | 导出录像 | POST | GET | 导出动作通常 POST，**改示例** |
| 4.3.30 | 配置VLAN | POST | GET | "配置"动作用 POST，**改示例** |
| 4.3.38 | 初始化指定逻辑盘 | POST | PATCH | "初始化"动作用 POST，**改示例** |
| 4.4.9 | 查询指定机柜电源信息 | GET | POST | "查询"用 GET，**改示例** |
| 4.10.2.12 | 查询南向SSL证书更新服务资源信息 | GET | PATCH | "查询"用 GET，**改示例** |

---

## 类型 B — 参数名拼写 / 空格

### B1. 参数名前后含空格（4 处，示例侧 JSON 实际写法）
...
|---|---|---|---|---|
| 4.2.4 | response | 示例多了一行 | `BobEnabled` | （疑似 `Enabled` 的笔误，需作者确认）|
| 4.2.20 | body | 示例 | `SubneMask` | `SubnetMask` |
| 4.2.22 | response | 表格 | `Lnks` | `Links` |
| 4.2.26 | response | 表格 | `Members@odata.nextLin` | `Members@odata.nextLink` |
| 4.2.35 | response | 示例 | `BaseVersioin` | `BaseVersion` |
| 4.2.39 | body | 表格 | `PackagaeName` | `PackageName` |
| 4.2.40 | response | 表格 | `MonitoringIntertval` | `MonitoringInterval` |
| 4.2.78 | response | 表格 | `EntryTyp` | `EntryType` |
| 4.3.19 | header | 表格 | `IsOnBoo` | `IsOnBoot` |
| 4.9.4 | response | 表格 | `MaxReaingRange` | `MaxReadingRange` |
| 4.10.5.2 / 4.10.6.2 | response | 表格 | `@odata.content` | `@odata.context` |

### B3. 大小写不一致（少量）

| section | 类别 | 表格 | 示例 | 建议 |
|---|---|---|---|---|
| 4.10.1.3 | response | `Error` / `Code` | `error` / `code` | 改表格，统一用小写（与 Redfish 规范一致）|
| 4.10.4.3 / 4.10.4.5 | response | `oem` / `huawei` | `oem` / `huawei` | 示例 JSON 用小写，但 Redfish 规范应大写 `Oem` / `Huawei`，**改示例**|
| 4.4.45 | response | `@Redfish.ActionInfo` | `@redfish.Actioninfo` | 示例大小写错，改示例 |

---

## 类型 C — Action / ActionInfo 前缀写法系统性不统一（60+ 处）

**现象**：表格表里写不带前缀的 `Manager.Reset`、`Redfish.ActionInfo`，示例 JSON 里则按 Redfish 规范写 `#Manager.Reset`、`@Redfish.ActionInfo`。
...
**建议**：表格表里所有 Action 字段名补 `#` 前缀，所有 ActionInfo 字段名补 `@` 前缀，与 Redfish 规范一致。可批量替换。
...
**影响**：之前表格解析工具不识别带空格的形式，多组响应表全部漏抓（4.2.39 漏 180 字段 / 4.4.27 漏一部分）。已在 extractor 侧修复。
...
**影响**：表格抓出 response=0，示例抓出 response=60+，对比时显示差异巨大。
