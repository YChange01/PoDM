...
from _defaults import DATA_DIR, OUTPUT_DIR, PODM_DOCX, PODM_DOCX_NAME, PODM_STEM  # noqa: E402
...
DEFAULT_OUTPUT = OUTPUT_DIR / f"{PODM_STEM}.resource-tree.interface-list.yaml"
...
def _resolve_io(argv: list[str]) -> tuple[Path, Path]:
    if len(argv) >= 2:
        ...
        if DATE_RE.match(first):
            ...
            out = OUTPUT_DIR / first / f"{PODM_STEM}.resource-tree.interface-list.yaml"
            if len(argv) > 2:
                out = Path(argv[2])
            return inp, out
        inp = Path(first)
...
def main(argv: list[str] | None = None) -> None:
    args = sys.argv if argv is None else ["extract_resource_tree_interfaces.py", *argv]
