#!/usr/bin/env python3
"""Promote a reviewed PoDManager-vs-BMC comparison workbook to the repository baseline."""
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="把人工复核后的 podm_bmc_summary.xlsx 提升为 baseline/podm_bmc/baseline.xlsx，并重新生成 manifest"
    )
    parser.add_argument(
        "date",
        nargs="?",
        help="输出日期目录，例如 20260610；默认读取 output/<date>/analysis/podm_bmc_summary.xlsx",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.source:
        ...
        source = OUTPUT_DIR / args.date / "analysis" / "podm_bmc_summary.xlsx"
    else:
        raise SystemExit("必须提供 date 或 --source")

    baseline_date = args.baseline_date or args.date or ""
    manifest = promote_podm_bmc_baseline(source, args.baseline_dir, baseline_date=baseline_date)
    workflow_dir = args.baseline_dir / WORKFLOW_ID
    print(f"已更新 PoDManager/BMC 基线 workbook: {workflow_dir / DEFAULT_BASELINE_WORKBOOK}")
    print(f"已更新 PoDManager/BMC 基线 manifest: {workflow_dir / DEFAULT_BASELINE_MANIFEST}")
    if manifest.get("previous_baseline_backup"):
        print(f"旧基线已归档: {manifest['previous_baseline_backup']}")
    print(f"PoDManager/BMC baseline SHA256: {manifest['sha256']}")


promote_reviewed_baseline = promote_podm_bmc_baseline
