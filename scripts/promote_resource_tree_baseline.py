#!/usr/bin/env python3
"""Promote a reviewed resource-tree comparison workbook to baseline."""
...
DEFAULT_BASELINE_WORKBOOK = "resource_tree_baseline.xlsx"
DEFAULT_BASELINE_MANIFEST = "resource_tree_baseline_manifest.json"
...
def unique_backup_dir(baseline_dir: Path, timestamp: str) -> Path:
    base = baseline_dir / f"backup_resource_tree_{timestamp}"
    if not base.exists():
        return base
    suffix = 1
    while True:
        candidate = baseline_dir / f"backup_resource_tree_{timestamp}_{suffix}"
...
def promote_resource_tree_baseline(
    ...
    if source_workbook.resolve() != target_workbook.resolve():
        backup_dir = archive_existing_baseline(baseline_dir, backup_timestamp or current_backup_timestamp())
        shutil.copy2(source_workbook, target_workbook)

    manifest = {
        "baseline_name": "resource_tree_baseline",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="把人工复核后的 resource_tree_summary.xlsx 提升为 baseline/resource_tree_baseline.xlsx"
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.source:
        ...
        source = OUTPUT_DIR / args.date / "analysis" / "resource_tree_summary.xlsx"
    else:
        raise SystemExit("必须提供 date 或 --source")

    baseline_date = args.baseline_date or args.date or ""
    manifest = promote_resource_tree_baseline(source, args.baseline_dir, baseline_date=baseline_date)
    print(f"已更新资源树基线 workbook: {args.baseline_dir / DEFAULT_BASELINE_WORKBOOK}")
    print(f"已更新资源树基线 manifest: {args.baseline_dir / DEFAULT_BASELINE_MANIFEST}")
    if manifest.get("previous_baseline_backup"):
        print(f"旧资源树基线已归档: {manifest['previous_baseline_backup']}")
    print(f"resource tree baseline SHA256: {manifest['sha256']}")
