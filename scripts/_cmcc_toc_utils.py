"""CMCC Word heading outline extraction helpers.

The CMCC document stores many section numbers in Word heading/outline metadata
instead of literal paragraph text. These helpers keep that logic shared by the
standalone TOC extractor and the CMCC interface extractors.
"""
...
from _cmcc_docx_utils import preferred_copy_pasted_text, read_source
from _doc_structure import _strip_trailing_pageno
...
def normalize_source_text(text: str) -> str:
    lines: list[str] = []
    heading_boundary = re.compile(r"(?<!^)(?<![\d.])(?=\d+(?:\.\d+)*\s+[^\s/{])")
...
def parse_toc_heading(line: str) -> tuple[str, str]:
    stripped = line.strip(" \t▪■●◆◇-")
...
def _is_unnumbered_front_heading(title: str) -> bool:
    return compact_title(title) in UNNUMBERED_FRONT_HEADINGS
...
def extract_toc(text: str) -> list[TocEntry]:
    ...
    for line_number, line in enumerate(normalize_source_text(text).splitlines(), start=1):
        section, title = parse_toc_heading(line)
        if _is_unnumbered_front_heading(title):
            continue
        append_entry(entries, seen, section, section.count(".") + 1, title, line_number)
    return entries
...
def extract_toc_from_docx(path: Path) -> list[TocEntry]:
    ...
    for child in body:
        ...
        section, title = parse_toc_heading(title_text)
        if section:
            ...
            title = title_text
            level = outline_level
        append_entry(entries, seen, section, level, title, paragraph_number)
