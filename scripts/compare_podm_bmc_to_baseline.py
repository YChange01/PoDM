#!/usr/bin/env python3
"""Compare PoDManager and BMC interface-list YAML files against a reviewed baseline."""
...
DEFAULT_BASELINE_MANIFEST = REPO_ROOT / "baseline" / "podm_bmc" / "manifest.json"
...
def compare_podm_bmc_to_baseline(
    baseline: Path,
    bmc_yaml: Path,
    podm_yaml: Path,
    output: Path,
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="基于人工确认 Excel 基线和新的 PoDManager/BMC interface-list YAML 生成带新增/删除/变更备注的 podm_bmc_summary.xlsx"
    )
    parser.add_argument("date", nargs="?", help="新输出日期目录，例如 20260610")
    parser.add_argument("--baseline", type=Path, default=DEFAULT_BASELINE, help="PoDManager/BMC 人工确认基线 workbook")
    parser.add_argument(
        "--baseline-manifest",
        type=Path,
    ...
    parser.add_argument("--output", type=Path, help="输出 xlsx；默认 output/<date>/analysis/podm_bmc_summary.xlsx")
    parser.add_argument("--report", type=Path, help="输出 JSON 更新报告；默认与 podm_bmc_summary.xlsx 同目录")
...
def default_yaml_paths(date: str) -> tuple[Path, Path]:
    ...
    return (
        out_dir / "bmc.interface-list.yaml",
        out_dir / "podm.interface-list.yaml",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.date:
        ...
        output = args.output
    report = args.report or (output.parent / "podm_bmc_update_report.json")
    ...
    update_report = compare_podm_bmc_to_baseline(args.baseline, bmc_yaml, podm_yaml, output)
...
update_summary_from_baseline = compare_podm_bmc_to_baseline
