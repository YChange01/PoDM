#!/usr/bin/env python3
"""Compare PoDManager resource-tree interfaces with PoDManager interface-list YAML."""
...
from _interface_compare import (  # noqa: E402
    InterfaceItem,
    build_match_indexes,
    clean,
    combine_notes,
...
DEFAULT_BASELINE = REPO_ROOT / "baseline" / "podm_resource_tree" / "baseline.xlsx"
DEFAULT_BASELINE_MANIFEST = REPO_ROOT / "baseline" / "podm_resource_tree" / "manifest.json"
...
def resource_tree_yaml_for_date(date: str) -> Path:
    return OUTPUT_DIR / date / "podm.resource-tree.interface-list.yaml"


def podm_yaml_for_date(date: str) -> Path:
    return OUTPUT_DIR / date / "podm.interface-list.yaml"
...
def compare_podm_resource_tree_to_baseline(
    baseline: Path | None,
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="对比 PoDManager 资源树接口和 PoDManager 接口清单，生成差异 workbook")
    ...
    parser.add_argument("--output", type=Path, help="输出 xlsx；默认 output/<date>/analysis/podm_resource_tree_summary.xlsx")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.date:
        ...
        output = args.output or (OUTPUT_DIR / args.date / "analysis" / "podm_resource_tree_summary.xlsx")
    else:
        ...
        output = args.output
    report = args.report or (output.parent / "podm_resource_tree_update_report.json")
    ...
    update_report = compare_podm_resource_tree_to_baseline(baseline, resource_tree_yaml, podm_yaml, output)
...
update_resource_tree_summary = compare_podm_resource_tree_to_baseline
